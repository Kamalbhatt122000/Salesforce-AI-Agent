...
class SalesforceBulkClient:
    ...
    def create_job(self, sobject, operation, external_id_field=None):
        ...
        response = requests.post(
            f"{self.base_url}/ingest/", headers=self._headers_json(), json=payload,
        )
    ...
    def get_job_status(self, job_id):
        ...
        response = requests.get(
            f"{self.base_url}/ingest/{job_id}", headers=self._headers_json(),
        )
    ...
    def wait_for_completion(self, job_id, poll_interval=15, max_wait=600):
        ...
        while elapsed < max_wait:
            ...
            print(f"   ⏳ Job {job_id}: {state} (processed: {processed}, failed: {failed})")
    ...
    def get_successful_results(self, job_id):
        ...
        response = requests.get(
            ...
            headers={"Authorization": f"Bearer {self.auth.access_token}", "Accept": "text/csv"},
        )
    ...
    def create_query_job(self, soql, operation="query"):
        ...
        response = requests.post(
            f"{self.base_url}/query/", headers=self._headers_json(), json=payload,
        )
